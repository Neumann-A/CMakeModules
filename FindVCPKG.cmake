...
if(VCPKG_TARGET_TRIPLET)
    set(VCPKG_PREFERRED_TRIPLET ${VCPKG_TARGET_TRIPLET} CACHE STRING "Prefered vcpkg triplet if it exists")
endif()
...
elseif(CMAKE_HOST_APPLE)
    set(VCPKG_PREFERRED_TRIPLET x64-osx CACHE STRING "Prefered vcpkg triplet if it exists")
elseif(CMAKE_HOST_UNIX)
    set(VCPKG_PREFERRED_TRIPLET x64-linux CACHE STRING "Prefered vcpkg triplet if it exists")
else()
...
function(vcpkg_get_triplet_path triplet outpath)
    macro(vcpkg_find_triplet tripbasepath intrip)
        if(EXISTS "${tripbasepath}/${intrip}")
            set(VCPKG_TARGET_TRIPLET "${intrip}" CACHE STRING "")
            message(STATUS "Found preferred triplet! Using: ${VCPKG_TARGET_TRIPLET}")
            set(${outpath} "${tripbasepath}" PARENT_SCOPE)
            return()
        endif()
    endmacro()

    foreach(triplet_path IN LISTS VCPKG_OVERLAY_TRIPLETS)
        vcpkg_find_triplet("${triplet_path}" "${triplet}")
    endforeach()
    vcpkg_find_triplet("${VCPKG_ROOT}/triplets" "${VCPKG_PREFERRED_TRIPLET}")
    message(FATAL_ERROR "Unable to find path to triplet! (${triplet})")
endfunction()
vcpkg_get_triplet_path(${VCPKG_PREFERRED_TRIPLET} VCPKG_TRIPLET_PATH)
